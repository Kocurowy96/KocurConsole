...
echo powershell -NoProfile -Command "$p = [Environment]::GetEnvironmentVariable('PATH','Machine'); $p = ($p -split ';' ^| Where-Object { $_ -ne '%INSTALL_DIR%' }) -join ';'; [Environment]::SetEnvironmentVariable('PATH',$p,'Machine')" >> "%INSTALL_DIR%\uninstall.bat"
